...
export const WeekView: React.FC<WeekViewProps> = ({ currentDate, events, onEventClick }) => {
  ...
  return (
    <>
      {/* Mobile View: A schedule-like list for the week */}
      <div className="sm:hidden h-full">
        <WeekScheduleView currentDate={currentDate} events={events} onEventClick={onEventClick} />
      </div>
      
      {/* Desktop View: The existing time grid */}
      <div className="hidden sm:block h-full overflow-y-auto bg-white">
       {/* Sticky Header Container: Holds both day headers and all-day events */}
      <div className="sticky top-0 bg-white z-10">
        {/* Day Headers */}
        <div className="grid grid-cols-[auto_repeat(7,1fr)] border-b border-gray-100">
                  ...
                  <div key={day.toString()} className="border-r border-gray-100 p-1 min-h-[2.5rem]">
                      {allDayEvents.map(event => (
                          <div 
                            ...
                            className="bg-blue-500 text-white rounded px-2 py-0.5 text-xs truncate mb-1 cursor-pointer hover:bg-blue-600 transition-colors" 
                            title={event.title}
                          >
                              {event.title}
                          </div>
            ...
            {events
              ...
              .map(event => (
                <div
                  key={event.id}
                  onClick={(e) => { e.stopPropagation(); onEventClick?.(event); }}
                  className="absolute left-2 right-2 p-1 bg-blue-100 text-blue-800 rounded-md text-xs overflow-hidden cursor-pointer hover:bg-blue-200 transition-colors hover:z-10"
